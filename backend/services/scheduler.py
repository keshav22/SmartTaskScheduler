...
def schedule_user_tasks(user_id: str):
    user_settings = supabase.table("users").select("*").eq("id", user_id).single().execute().data
    ...
    tasks = supabase.table("tasks").select("*").eq("user_id", user_id).eq("status", "pending").execute().data
    ...
    floating_tasks.sort(key=lambda x: (x['deadline'] or '9999', -int(x['priority'] or 0)))
